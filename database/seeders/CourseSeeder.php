    ...
    public function run(): void
    {
        ...
        $student_courses = $student_levels->flatMap(function (Level $level) use ($student_subjects) {
            return $student_subjects->map(function (Subject $subject) use ($level) {
                return [
                    'name' => $subject->name . ' ' . $level->level . ' 2026',
                    'description' => $subject->name . ' ' . $level->level . ' 2026',
                    'status' => 'published',
                    ...
                    'slug' => Str::slug($subject->name . ' ' . $level->level . ' 2026')
                ];
            });
        });
        ...
        Course::insert($student_courses->toArray());
    }
}
